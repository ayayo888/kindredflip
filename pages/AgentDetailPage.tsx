...
const AgentDetailPage: React.FC = () => {
  ...
  const name = formatName(agentName);
  
  // SEO & Meta
  useEffect(() => {
    document.title = `${name} Spreadsheet 2026 | Verified Links & Coupon Codes List`;
    
    const setMetaTag = (tagName: string, content: string) => {
        let element = document.querySelector(`meta[name="${tagName}"]`);
        if (!element) {
    ...
    const desc = `The most comprehensive ${name} Spreadsheet. Access 3,000+ QC-verified finds for Sneakers, Streetwear, and Tech. Includes guide on How to Order via ${name}.`;

    setMetaTag('description', desc);
    setMetaTag('keywords', `${name} spreadsheet, ${name} agent, how to order ${name}, ${name} qc, chinese fashion agent, ${name} shipping`);
      ...
      <div className="bg-kf-offwhite border-b-2 border-gray-200">
        <div className="max-w-7xl mx-auto px-4 py-3">
            <nav className="flex items-center gap-2 text-xs md:text-sm font-mono font-bold uppercase text-gray-500">
                ...
                <Link to="/agent" className="hover:text-black hover:underline decoration-2 underline-offset-2">
                    Agent Database
                </Link>
                <ChevronRight className="w-3 h-3 text-gray-300" />
          ...
          <div className="max-w-7xl mx-auto">
              <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-6 border-l-4 border-black">
                  {GRID_CATEGORIES.map((cat, idx) => (
                      <Link 
                        to={`/gold?category=${cat.name.toLowerCase()}`} 
                        key={idx}
