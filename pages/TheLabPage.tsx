...
import { Filter, TestTube, XCircle, CheckCircle, Scale, AlertTriangle, ArrowRight } from 'lucide-react';
import { LAB_ITEMS } from '../constants';

const CATEGORIES = ['ALL', 'SHOES', 'HOODIES', 'TECH', 'BAGS', 'ACCESSORIES'];

const TheLabPage: React.FC = () => {
  const [activeCategory, setActiveCategory] = useState('ALL');
  const [filterStatus, setFilterStatus] = useState<'ALL' | 'PASS' | 'FAIL'>('ALL');

  // Update Page Title
  useEffect(() => {
    document.title = "CNfans Reviews & In-Hand QC Tests 2026 _The Lab";
    return () => {
  ...
  return (
    <div className="bg-kf-offwhite min-h-screen pb-24 md:pb-10 font-sans">
      ...
      {/* 1. SECTION: THE HERO */}
      <section className="bg-white border-b-[3px] border-black overflow-hidden relative">
        <div className="max-w-7xl mx-auto flex flex-col md:flex-row min-h-[500px]">
          ...
          <div className="w-full md:w-1/2 p-6 md:p-12 flex flex-col justify-center relative z-20">
            <div className="inline-flex items-center gap-2 mb-6">
                ...
                <span className="font-mono font-bold text-sm tracking-widest text-gray-500 typewriter">
          ...
          {/* Right: Visuals (Mobile: Background / Desktop: 3D Element) */}
          <div className="w-full md:w-1/2 relative bg-gray-100 border-t-[3px] md:border-t-0 md:border-l-[3px] border-black flex items-center justify-center overflow-hidden min-h-[300px] md:min-h-auto">
             
             {/* 3D Box Simulation */}
             <div className="relative w-48 h-48 md:w-64 md:h-64 animate-bounce-slow">
                <div className="absolute inset-0 border-[3px] border-black bg-white transform rotate-3 z-10 shadow-hard flex items-center justify-center">
                    <TestTube className="w-24 h-24 text-kf-blue" />
                </div>
                <div className="absolute inset-0 border-[3px] border-black bg-kf-yellow transform -rotate-6 z-0"></div>
      ...
      <section className="border-b-[3px] border-black bg-white">
        <div className="max-w-7xl mx-auto flex flex-col md:flex-row">
            {/* Stats Label Area */}
            <div className="w-full md:w-[20%] p-4 bg-black text-white flex flex-row md:flex-col justify-between items-center md:items-start md:justify-center border-b-[3px] md:border-b-0 md:border-r-[3px] border-black">
                <div>
            ...
            <div className="w-full md:w-[80%] h-12 md:h-auto flex relative group">
                {/* Passed Segment */}
                <div className="w-[20%] bg-kf-green h-full flex items-center justify-center border-r-[3px] border-black relative overflow-hidden">
      ...
      <section className="sticky top-[68px] z-30 bg-white border-b-[3px] border-black shadow-md">
         <div className="max-w-7xl mx-auto">
            <div className="flex flex-col md:flex-row">
                
                {/* Primary Filters (Pass/Fail) - Fixed left on Mobile scroll */}
                <div className="flex shrink-0 border-b-[3px] md:border-b-0 md:border-r-[3px] border-black">
                    <button 
                        onClick={() => setFilterStatus('ALL')}
                        className={`flex-1 md:flex-none px-6 py-4 font-black uppercase text-sm hover:bg-black hover:text-white transition-colors border-r-[3px] border-black md:border-r-0 ${filterStatus === 'ALL' ? 'bg-black text-white' : ''}`}
                    >
                        All
                    </button>
                    <button 
                        onClick={() => setFilterStatus('PASS')}
                        className={`flex-1 md:flex-none px-6 py-4 font-black uppercase text-sm hover:bg-kf-green hover:text-black transition-colors border-r-[3px] border-black md:border-r-0 ${filterStatus === 'PASS' ? 'bg-kf-green' : ''}`}
                    >
                ...
                <div className="flex-1 overflow-x-auto whitespace-nowrap scrollbar-hide p-2 md:p-0 flex items-center md:flex-wrap bg-gray-50 md:bg-white">
                    {CATEGORIES.map(cat => (
                        <button
                            ...
                            className={`
                                mx-1 md:mx-0 md:px-6 md:py-4 px-4 py-2 rounded-full md:rounded-none border-[3px] border-black md:border-0 md:border-r-[3px] md:border-b-0
      ...
      <div className="fixed bottom-4 left-4 right-4 md:hidden z-50">
          <Link to="/gold" className="block w-full bg-black text-kf-yellow border-[3px] border-white shadow-hard-lg rounded-xl p-4 flex items-center justify-between group active:scale-95 transition-transform">
      ...
      <div className="hidden md:block max-w-7xl mx-auto px-4 mt-12 mb-12">
          <div className="bg-kf-yellow border-[3px] border-black p-12 flex items-center justify-between shadow-hard relative overflow-hidden">
               ...
               <Link to="/gold" className="relative z-10 bg-black text-white px-10 py-5 font-black text-xl border-[3px] border-white shadow-hard hover:translate-x-1 hover:translate-y-1 hover:shadow-none transition-all uppercase">
                   Access Gold List 🏆
               </Link>
          </div>
      </div>
